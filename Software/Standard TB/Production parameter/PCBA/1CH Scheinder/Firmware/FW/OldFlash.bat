...
SET JLINKSN=59800894
rem SET JLINKSN=59800894
SET PROGRAM_BOOT_SIGNED=p2168_nodon_bootloader_fw_efr32.production.hex
rem parameter 1 is production applicative FW .HEX
rem parameter 2 is Security txt files Path
...
%5 security writekey --sign %3\WISER_POWER_MICROMODULE_Signature-pub.pem --device=EFR32MG21A010F1024IM32 --noprompt
timeout /t 1 /nobreak
%5 security readkey --sign --device=EFR32MG21A010F1024IM32

REM Write Secure Boot settings and anti-tamper configuration
%5 security writeconfig --configfile %3\efr32mg21_security.json --device=EFR32MG21A010F1024IM32 --noprompt
timeout /t 2 /nobreak
